...
// 流量信息匹配正则（Sub-Store 常见的流量节点名）
const TRAFFIC_KEYWORDS = /(流量|到期|剩余|GB|已用|Expire|Usage|Traffic|Standard|Used|Total)/i;

const HARDCODED_GROUP_NAMES = [
    "Account Info", "CDN", "AI", "Telegram", "YouTube", "Bilibili", "Netflix", "Spotify",
    "TikTok", "E-Hentai", "PikPak", "Truth Social", "Bahamut", "Crypto",
    "SSH(port 22)", "Sogou", "AdBlock", "GLOBAL"
];
...
const countriesMeta = {
    "Macau": { pattern: "(?i)(澳门|MO|Macau|Macao|🇲🇴)", icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Macao.png" },
    "Hong Kong": { pattern: "(?i)(香港|HK|Hong Kong|HongKong|hongkong|🇭🇰)", icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Hong_Kong.png" },
    "Taiwan": { pattern: "(?i)(台湾|TW|Taiwan|新北|彰化|🇹🇼)", icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Taiwan.png" },
    "Singapore": { pattern: "(?i)(新加坡|SG|Singapore|狮城|🇸🇬)", icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Singapore.png" },
    "Japan": { pattern: "(?i)(日本|JP|Japan|东京|大阪|埼玉|🇯🇵)", icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Japan.png" },
    "Korea": { pattern: "(?i)(韩国|KR|Korea|KOR|首尔|韓|🇰🇷)", icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Korea.png" },
    "United States": { pattern: "(?i)(美国|US|United States|USA|🇺🇸)", icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/United_States.png" },
    "Canada": { pattern: "(?i)(加拿大|Canada|🇨🇦)", icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Canada.png" },
    "United Kingdom": { pattern: "(?i)(英国|United Kingdom|UK|伦敦|London|🇬🇧)", icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/United_Kingdom.png" },
    "Australia": { pattern: "(?i)(澳大利亚|澳洲|Australia|🇦🇺)", icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Australia.png" },
    "Germany": { pattern: "(?i)(德国|Germany|🇩🇪)", icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Germany.png" },
    "France": { pattern: "(?i)(法国|France|🇫🇷)", icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/France.png" },
    "Russia": { pattern: "(?i)(俄罗斯|RU|Russia|🇷🇺)", icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Russia.png" },
    "Thailand": { pattern: "(?i)(泰国|Thailand|🇹🇭)", icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Thailand.png" },
    "Netherlands": { pattern: "(?i)(荷兰|Netherlands|Holland|🇳🇱)", icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Netherlands.png" },
    "India": { pattern: "(?i)(印度|India|🇮🇳)", icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/India.png" },
    "Malaysia": { pattern: "(?i)(马来西亚|Malaysia|🇲🇾)", icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Malaysia.png" }
};
...
function parseCountries(proxies) {
    ...
    for (const p of proxies) {
        const t = p.name || "";
        // 排除掉流量节点，不计入国家统计
        if (TRAFFIC_KEYWORDS.test(t)) continue;
...
function buildCountryProxyGroups({ countries: e, loadBalance: o }) {
    ...
    for (const l of e) {
        ...
        const i = {
            ...
            "include-all": !0,
            filter: e.pattern,
            "exclude-filter": TRAFFIC_KEYWORDS.source, // 排除流量信息节点
            type: s
        };
...
function buildProxyGroups({ countries: t, countryProxyGroups: o, defaultProxies: n, defaultProxiesDirect: s, defaultFallback: i, trafficNodes: trafficNodes }) {
    ...
    // 如果识别到流量节点，把 Account Info 组加在 Proxies 下方
    if (trafficNodes.length > 0) {
        groups.push({
            name: PROXY_GROUPS.ACCOUNT,
            icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/About.png",
            type: "select",
...
function main(e) {
    ...
    // 识别流量节点
    const trafficNodes = proxies.filter(p => TRAFFIC_KEYWORDS.test(p.name)).map(p => p.name);
    // 识别真实代理节点
    const realProxies = proxies.filter(p => !TRAFFIC_KEYWORDS.test(p.name));
    
    const t = { proxies: proxies };
    const o = parseCountries(proxies);
