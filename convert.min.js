...
const HARDCODED_GROUP_NAMES = [
    "前置代理", "静态资源", "AI", "Telegram", "YouTube", "Bilibili", "Netflix", "Spotify",
    "TikTok", "E-Hentai", "PikPak", "Truth Social", "Bahamut", "Crypto",
    "SSH(22端口)", "搜狗输入法", "广告拦截", "GLOBAL"
];
...
function buildBaseLists({ landing: e, lowCost: t, countryGroupNames: o }) {
    ...
    return {
        defaultProxies: buildList(PROXY_GROUPS.MANUAL, o, t && PROXY_GROUPS.LOW_COST, PROXY_GROUPS.DIRECT),
        defaultProxiesDirect: buildList(PROXY_GROUPS.DIRECT, o, t && PROXY_GROUPS.LOW_COST, PROXY_GROUPS.MANUAL),
        defaultSelector: r,
...
const countriesMeta = {
    "澳门": { pattern: "(?i)(澳门|MO|Macau|Macao|🇲🇴)", icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Macao.png" },
    "香港": { pattern: "(?i)(香港|HK|Hong Kong|HongKong|hongkong|🇭🇰)", icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Hong_Kong.png" },
    "台湾": { pattern: "(?i)(台湾|TW|Taiwan|新北|彰化|🇹🇼)", icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Taiwan.png" },
    "新加坡": { pattern: "(?i)(新加坡|SG|Singapore|狮城|🇸🇬)", icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Singapore.png" },
    "日本": { pattern: "(?i)(日本|JP|Japan|东京|大阪|埼玉|🇯🇵)", icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Japan.png" },
    "韩国": { pattern: "(?i)(韩国|KR|Korea|KOR|首尔|韓|🇰🇷)", icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Korea.png" },
    "美国": { pattern: "(?i)(美国|US|United States|USA|🇺🇸)", icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/United_States.png" },
    "加拿大": { pattern: "(?i)(加拿大|Canada|🇨🇦)", icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Canada.png" },
    ...
    "澳大利亚": { pattern: "(?i)(澳大利亚|澳洲|Australia|🇦🇺)", icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Australia.png" },
    "德国": { pattern: "(?i)(德国|Germany|🇩🇪)", icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Germany.png" },
    "法国": { pattern: "(?i)(法国|France|🇫🇷)", icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/France.png" },
    "俄罗斯": { pattern: "(?i)(俄罗斯|RU|Russia|🇷🇺)", icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Russia.png" },
    "泰国": { pattern: "(?i)(泰国|Thailand|🇹🇭)", icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Thailand.png" },
    "荷兰": { pattern: "(?i)(荷兰|Netherlands|Holland|🇳🇱)", icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Netherlands.png" },
    "印度": { pattern: "(?i)(印度|India|🇮🇳)", icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/India.png" },
    "马来西亚": { pattern: "(?i)(马来西亚|Malaysia|🇲🇾)", icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Malaysia.png" }
};
...
function parseCountries(e) {
    const t = e.proxies || [],
        ...
        n = {};
    const countryOrder = ["澳门", "香港", "台湾", "新加坡", "日本", "韩国", "美国", "加拿大", "英国", "澳大利亚", "德国", "法国", "俄罗斯", "泰国", "荷兰", "印度", "马来西亚"];
    for (const e of countryOrder)
        if (countriesMeta[e]) n[e] = new RegExp(countriesMeta[e].pattern.replace(/^\(\?i\)/, ""), "i");
    for (const e of t) {
        const t = e.name || "";
        if (!o.test(t))
            for (const e of countryOrder) {
...
function buildProxyGroups({ landing: e, countries: t, countryProxyGroups: o, lowCost: r, defaultProxies: n, defaultProxiesDirect: s, defaultSelector: l, defaultFallback: i }) {
    ...
    e ? {
        ...
        type: "select",
        proxies: p ? ["美国节点", PROXY_GROUPS.MANUAL] : n
    }, {
        name: "Bahamut",
        icon: "https://cdn.jsdmirror.com/gh/Koolson/Qure@master/IconSet/Color/Bahamut.png",
        type: "select",
        proxies: a ? ["台湾节点", PROXY_GROUPS.MANUAL, PROXY_GROUPS.DIRECT] : n
    }, {
        name: "Crypto",
...
function deduplicateProxies(proxies, reservedNames) {
...
function main(e) {
    ...
    // Create reserved names list including all potential group names
    const reserved = new Set([
        ...Object.values(PROXY_GROUPS),
        ...HARDCODED_GROUP_NAMES,
        ...n
    ]);

    // Deduplicate proxies against each other AND reserved group names
    deduplicateProxies(t.proxies, reserved);

    const r = hasLowCost(t);
    const s = stripNodeSuffix(n);
    const { defaultProxies: l, defaultProxiesDirect: i, defaultSelector: a, defaultFallback: c } = buildBaseLists({ landing: landing, lowCost: r, countryGroupNames: n });
    const p = buildCountryProxyGroups({ countries: s, landing: landing, loadBalance: loadBalance });
    const u = buildProxyGroups({ landing: landing, countries: s, countryProxyGroups: p, lowCost: r, defaultProxies: l, defaultProxiesDirect: i, defaultSelector: a, defaultFallback: c });
    ...
    }), Object.assign(t, { "proxy-groups": u, "rule-providers": ruleProviders, rules: g, sniffer: snifferConfig, dns: fakeIPEnabled ? dnsConfigFakeIp : dnsConfig, "geodata-mode": !0, "geox-url": geoxURL }), t;
}
