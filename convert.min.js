...
// 流量信息关键词：增加 建议|重置|官网|套餐|流量|剩余
const TRAFFIC_KEYWORDS = /(建议|重置|官网|套餐|流量|剩余|到期|GB|Expire|Usage|Traffic|Standard|Used|Total)/i;
// 白名单：包含以下字符的依然视为普通节点（解决赞助节点问题）
...
function buildCountryProxyGroups({ countries: e, loadBalance: o }) {
    ...
    for (const l of e) {
        ...
        const i = {
            ...
            "include-all": !0,
            filter: e.pattern,
            "exclude-filter": "建议|重置|官网|套餐|流量|剩余|到期|GB", // 再次加固排除
            type: s
        };
...
function buildProxyGroups({ countries: t, countryProxyGroups: o, defaultProxies: n, defaultProxiesDirect: s, defaultFallback: i, trafficNodes: trafficNodes, realProxyNames: realProxyNames }) {
    ...
    const groups = [{
        ...
        type: "select",
        proxies: realProxyNames // 强制只显示真实节点
    }];
