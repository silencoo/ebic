...
// 流量信息关键词：增加 “防丢”
const TRAFFIC_KEYWORDS = /(建议|重置|官网|套餐|流量|剩余|到期|防丢|GB|导航|更新|Expire|Usage|Traffic|Standard|Used|Total)/i;
// 白名单：包含以下字符的依然视为普通节点
...
function buildProxyGroups({ countries: t, countryProxyGroups: o, defaultProxies: n, defaultProxiesDirect: s, defaultFallback: i, trafficNodes: trafficNodes, realProxyNames: realProxyNames }) {
    ...
    const aiSpecificProxies = realProxyNames.filter(name => /(residential|resident|家宽)/i.test(name));
    ...
    const commonGroups = [{
        ...
        type: "select",
        proxies: [...aiSpecificProxies, PROXY_GROUPS.MANUAL, ...realProxyNames]
    }, {
        name: "Telegram",
